...
namespace finitediff {
	...
	void calculate_weights(const Real_t * const __restrict grid, const unsigned len_g,
		const unsigned max_deriv, Real_t * const __restrict weights, const Real_t around = 0) {
		// Parameters
		// ----------
	...
	template <typename Real_t>
	void populate_weights(const Real_t z, const Real_t * const __restrict x, const int nd,
		const int m, Real_t * const __restrict c) {
		// Parameters
		// ----------
	...
	void apply_fd(const int nin, const int maxorder,
		const Real_t * const __restrict xdata,
		const Real_t * const __restrict ydata,
		const Real_t xtgt,
		Real_t * const __restrict out){
	...
	template <typename Real_t>
	void calculate_weights_optim(const Real_t * const __restrict grid, const unsigned len_g,
		const unsigned max_deriv, Real_t * const __restrict weights, const Real_t around = 0) {
		if (len_g < max_deriv + 1){
			throw std::logic_error("size of grid insufficient");
		}
